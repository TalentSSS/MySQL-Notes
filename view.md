...
**存储程序**(*stored program*)，一种类型的存储对象。形式多样，可以按需调用，可以修改表或达到预定时间时自动执行。
...
* 存储过程(*stored procedure*)
    ...
    不直接返回结果，可以用来完成一般的运算，生成可以传递到客户端的**结果集**
...
显式指定参数类型的方法是，在参数列表里的参数名前面使用IN、OUT或INOUT。如果没有为参数指定类型，则其默认类型为IN。

在调用过程时给OUT或INOUT指定一个变量名即可使用。过程可以设置参数值， 相应的变量将在过程返回时获得那个值。

如果想让某个存储过程返回多个结果值，可以用OUT和INOUT。
...
在调用此过程时，把各个参数替换成相应的用户定义变量，过程将把统计值放到这些参数里，返回之后即可获得
...
关键字IN、OUT和INOUT都不能用于存储函数、触发器和事件。对于存储函数，所有参数都像IN参数。
...
* 可以检查或修改将被插入或用来更新行的那些新的数据值，从而实现数据完整性约束，对输入数据进行过滤

* 可以基于某个表达式来为列提供默认值，甚至可以为那些只能使用常量默认值进行定义的列类型提供值

* 可以在行删除或更新之前先检查行的当前内容，这种能力能完成许多任务，如记录已有行的更改情况

在触发器的定义里，需要指明：

* 触发它的语句类型(INSERT、UPDATE或DELETE)
...
trigger_name是触发器的名字；tbl_name是与触发器相关联的那个表的名字。

在给触发器命名时，最好在名字中体现触发器的用途和关联的表，如bi_tbl_name表示的是 一个与tbl_name表有关的BEFORE INSERT触发器；而ai_tbl_name表示的则是一个与 tbl_name表有关的AFTER_INSERT触发器。

trigger_stmt是触发器的语句体，即在触发器被激活时需要执行的语句。在触发器的语句体里，可以使用**NEW**.*col_name*语法来引用在INSERT或UPDATE触发器里将被插入或修改的那个新行里的列。类似地，**OLD**.*col_name*语法可以用来引用在DELETE或UPDATE触发器里将被删除或修改的原行里的列。如果想要用BEFORE触发器改变列值，而且想在值存储到表中之前改变它，那么可以使用**SET** NEW.*col_name* = value。

在下面这个示例里，表t的INSERT语句有一个名为bi_t的触发器。其中，表t有一个整型列percent，用于存储百分比值（范围为0-100）；另外还有一个DATETIME列。该触发器使用的是BEFORE，因此它可以在数据值被插入进表之前对它们进行检查。
...
```mysql
mysql> INSERT INTO t(percent) VALUES(-2);
mysql> INSERT INTO t(percent) VALUES(30);
...
默认情况下，事件调度器并不会运行，因此必须先启用它才能使用事件。请把下面两行内容放到某个在服务器启动时会被读取的选项文件里
...
如果你停止了调度器，所有事件都不会运行。也可以让事件调度器保持运行，但禁用各个事件。

**如果在服务器启动时把变量event_scheduler设置成DISABLED, 那么在运行时只能查看其状态，而不能更改它。你可以创建事件，但它们不会执行**。

事件调度器会把执行情况写到服务器的错误日志里，通过它，你可以查看到事件调度器在做什么。
...
如果想创建一个只执行一次的事件，则应该使用AT调度类型，而不要使用EVERY。创建一个在一个小时后只执行一次的事件：
...
在定义视图、存储程序时，需要定义一些以后执行的对象，如果以后执行这些操作的用户不是之前创建它们的用户，就需要考虑：服务器在执行时应该使用什么样的安全上下文来检查访问权限呢？也就是要考虑如何检查调用者的权限

默认情况下，服务器会使用定义该对象的那个用户的账户，将视图、存储程序的权限授予给其他访问者，使其他人能够以定义者的权限访问数据。
...
在定义存储程序或视图时，在该对象的CREATE语句里包含一 个DEFINER= account子句，可以显式指定定义者。这样，这里列出的账户就能跟定义者完全一样，从而达到在执行时检查访问权限的目的。例如：
...
对于视图和存储例程（包括存储函数和存储过程），还可以使用SQL SECURITY特性，用于实现对执行时访问检查的附加控制。SQL SECURITY特性的允许值为DEFINER（以定义者的权限执行）或INVOKER（以对象调用者的权限执行）。 

适合使用SQL SECURITY INVOKER的场合是：只想以调用者所拥有的权限来执行视图或存储程序。例如，下面这个视图将访问mysql数据库里的某个表，但是以调用者的权限来运行。这样 一来，如果调用者本人无权访问mysql.user表，那么即使他访问这个视图，也无法突破权限限制。
